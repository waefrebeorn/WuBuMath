...
- A005 orientation flag (horizontal vs vertical sweep) absent `open`
...
- B003 no band registry ("which coordinates are invisible") `open`
...
- C002 no probability-flow ODE integrator (no euler/rk4/ode loop in train step) `open` DA: research nodes 2.1/2.2 define what's required
- C003 parallel transport never called from training code (module exists, unwired) `open`
...
- D002 no InfoNCE implementation `open`
- D003 no geodesic-distance similarity head for embeddings `open`
...
- E001 no STFT/ISTFT module in C11 (Zephyr-HD lives in Python only) `open`
...
- F003 no property-harness framework (invariant registry, fuzz seeds) `open`
...
- G001 decoder IS coordinate-addressable but no "decode at N" public API taking target res `open`
